...
class Traveltimes_NLL:
    ...
    def load_header(self):
        ...
        lines = f.readlines()
        f.close()
        self.nx, self.ny, self.nz = [ int(x)   for x in lines[0].split()] # number of grid points in x,y,z direction
        self.x0, self.y0, self.z0 = [ float(x) for x in lines[1].split()] # origin of x,y,z coordinate (km)
        self.dx, self.dy, self.dz = [ float(x) for x in lines[2].split()] # grid spacing in x,y,z (km)
        self.lat0, self.lon0, self.el0 = [ float(x) for x in lines[3].split()] # grid origin latitude,longitude,elevation
        self.x = self.x0+(num.arange(0,self.nx)*self.dx) # x axis
        self.y = self.y0+(num.arange(0,self.ny)*self.dy) # y axis
        self.z = self.z0+(num.arange(0,self.nz)*self.dz) # z axis
    ...
    def load_traveltimes(self, phase, label, precision='single'):
        t={}
        for sta in self.db_stations:
            ...
            if (precision=='single'):
                ...
                sys.exit()
        print(f'Successfully loaded TRAVELTIME data for {len(t)} channels (phase {phase})')
        return t

    def ttdb_reduce(self,tt,l_lim,u_lim,zlim=[]):
        latref=self.lat0; lonref=self.lon0; eleref=self.el0
...
if __name__ == "__main__":
    db_path = '../NLL/FLEGREI_stations/nll_grid'
    hdr_filename = 'header.hdr'
    precision='single'
    label = 'time'

    tobj = Traveltimes_NLL(db_path, hdr_filename)
    tp = tobj.load_traveltimes('P', label=label, precision=precision)
    
    print(f"Stations: {list(tp.keys())}")
    print(f"Loaded travel times for {len(tp)} stations")
    
    # Example: access tt for first station
    first_station = list(tp.keys())[0]
    tt = tp[first_station]
    print(f"\nP travel times for station {first_station}:")
    print(f"Travel times length: {tt.shape}")
    print(f"\nSome travel times: {tt[0:10]}")
